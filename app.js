...
function normalizeImport(o) {
  return {
    numero_ot: o.numero_ot || o['n° ot'] || o['n ot'] || o.ot || '',
    nom: o.nom || o.titre || '',
    date_fin: formatDateForSupabase(o.date_fin || o['date de fin'] || ''),
    emplacement: o.emplacement || o.lieu || '',
    autre: o.autre || o.commentaire || '',
    statut: 'Disponible',
...
async function confirmImport(){
  ...
  await log('Import OT', `${rows.length} OT traités. Les doublons ont été ignorés.`);
  await loadAll();
  state.importRows = [];
  msg(`${rows.length} OT traités. Les doublons déjà présents ont été ignorés.`);
}{const existing=new Set(state.ots.map(o=>o.numero_ot)); const rows=state.importRows.filter(r=>!existing.has(r.numero_ot)); const skipped=state.importRows.length-rows.length; if(!rows.length) return err('Aucun nouvel OT à importer.'); const {error}=await sb.from('ots').insert(rows); if(error) return err(error.message); await log('Import OT',`${rows.length} ajoutés, ${skipped} ignorés`); await loadAll(); state.importRows=[]; msg(`${rows.length} OT importés. ${skipped} doublons ignorés.`)}
