...
function statusOf(o){  if(o.statut==='Incomplet') return 'Incomplet'; if(o.statut==='Pris') return 'Pris'; if(o.date_fin && o.date_fin < today()) return 'Retard'; return 'Disponible'; }
...
async function saveMissingArticle(e){
  ...
  await sb
  .from('ots')
  .update({ statut:'Incomplet' })
  .eq('numero_ot', payload.numero_ot);

await loadOTs();
  await loadMissingArticles();
  msg('Article manquant ajouté. OT passé en Incomplet.');
}
